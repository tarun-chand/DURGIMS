...
def productCompanyFilter(request):
    list_all_com = ProductModelMaster.objects.select_related('product_com').filter(product_com__product_cat_id=request.GET.get('procat'))
...
def productModelRedirect(request):
    ...
    list_all_pcm = ProductCompanyMaster.objects.select_related('product_cat')
...
def productModelSubmit(request):
    ...
    pcm.product_com = productcomname
...
def productModelUpdateRedirect(request):
    list_all_pcm = ProductCompanyMaster.objects.select_related('product_cat').filter(product_cat_id__product_type="Non-Consumable")
    list_all_pmn = ProductModelMaster.objects.all()
...
def productDetails(request):
    ...
    list_all_pc = ProductCategoryMaster.objects.all()
    list_all_memo = PurchaseDetails.objects.all()
    return render(request,"master/product/productDetails.html",{'list_all_pd':list_all_pd,'list_all_pc':list_all_pc,'list_all_memo':list_all_memo})
...
def productDetailsSubmit(request):
    ...
    productnew = request.POST.get('productnew')
    purchasesource = ProductPuchaseDetails.objects.get(propur_id=request.POST.get('propurfrom'))

    is_exists = ProductDetails.objects.filter(product_mod=productmodel,product_serialno=serialno,propur=purchasesource).exists()
    if is_exists:
...
def productDetailsUpdate(request):
    ...
    print("pd.product_cat_id ---------- ",pd.product_cat_id)
    ...
    pd.current_quantity = request.POST.get('quantity')
    print("request.POST.get('quantity') ==== ",request.POST.get('quantity'))
    pd.cartridge_toner = request.POST.get('toner')
    pd.remarks = request.POST.get('remarks')
    pd.product_new_or_open = request.POST.get('productnew')
    pd.save()
